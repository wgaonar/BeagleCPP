...
void PWM::InitPWMPin()
{
  ...
  {
  case P8_13:
    chipName = PWM_PATH + EHRPWM2_PATH + "pwmchip6/";
    name = EHRPWM2_PATH + "pwmchip6/pwm1/";
    interfaceNumber = 1;
    break;
  case P8_19:
    chipName = PWM_PATH + EHRPWM2_PATH + "pwmchip6/";
    name = EHRPWM2_PATH + "pwmchip6/pwm0/";
    interfaceNumber = 0;
    break;
  case P9_14:
    chipName = PWM_PATH + EHRPWM1_PATH + "pwmchip3/";
    name = EHRPWM1_PATH + "pwmchip3/pwm0/";
    interfaceNumber = 0;
    break;
  case P9_16:
    chipName = PWM_PATH + EHRPWM1_PATH + "pwmchip3/";
    name = EHRPWM1_PATH + "pwmchip3/pwm1/";
    interfaceNumber = 1;
    break;
  case P9_21:
    chipName = PWM_PATH + EHRPWM0_PATH + "pwmchip1/";
    name = EHRPWM0_PATH + "pwmchip1/pwm1/";
    interfaceNumber = 1;
    break;
  case P9_22:
    chipName = PWM_PATH + EHRPWM0_PATH + "pwmchip1/";
    name = EHRPWM0_PATH + "pwmchip1/pwm0/";
    interfaceNumber = 0;
    break;
  default:
    break;
  ...
  this->Export(interfaceNumber);
  this->Enable();
  this->SetPeriod(period);
}

/*
   Private method to export the PWM on the pin.
   It is used in the constructor
   @params int: the interface number of the pwm chip (0/1)
   @return int: 1 enable has succeeded / throw an exception if not
*/
int PWM::Export(int interfaceNumber)
{
  if (WriteFile(chipName, "export", interfaceNumber) != 1)
  {
    perror("Error trying to export the PWM interface on the pin");
    throw PWM_Exception("Error in the 'Export' method");
  }
  else 
    return 1;
}
